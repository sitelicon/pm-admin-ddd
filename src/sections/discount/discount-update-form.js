...
import { Delete, Trash01 } from '@untitled-ui/icons-react';
...
export const DiscountUpdateForm = ({ discount, refetch }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      from: discount.from ? new Date(discount.from) : null,
      to: discount.to ? new Date(discount.to) : null,
      stores: discount.stores?.map((store) => store.id) || [],
      customerGroups: discount.customer_groups?.map((group) => group.id) || [],
      categories:
        discount.categories?.map((category) => category.category_id) || [],
      applyTo: discount.type_id,
      discountValue: discount.value,
      excludeOnSale: discount.exclude_on_sale_products === 1,
      freeShipping: discount.free_shipping === 1,
      submit: null,
    },
    validationSchema,
    onSubmit: async (values, helpers) => {
      try {
        const response = await discountsApi.updateDiscount({
          id: discount.id,
  ...
  const handleDelete = async () => {
    try {
      if (window.confirm('¿Está seguro de eliminar este descuento?')) {
        ...
        router.push(paths.discounts.index);
      }
    } catch (err) {
      console.error(err);
      toast.error('¡Algo ha ido mal!');
